<x-layout>
    <x-slot name="headings">
        <div class="flex items-center justify-between" style="margin-top:20px">
            <div class="text-blue-500">La Commune Urbaine : {{$nomCom->cr_name}}</div>
            <div style="margin-left:20px">
                <x-button href="/names/create">Create</x-button>
            </div>
        </div>
    </x-slot>
    <form method="POST" action="/Regisseur/{{$nomCom}}" class="max-w-sm mx-auto mt-6 space-y-4">
        @csrf
        <div class="flex justify-content-around">
            <div>
                ...
                <select id="regisseurs" name="regisseurs"  class="form-select form-select-lg mb-3" aria-label="Large select example">
                    ...
                    @if (!empty($regisseurs))
                            <option value="{{$regisseurs->id}}">{{$regisseurs->name}}</option>
                    @else
    ...
    <form method="POST" action="/show/{{$cr_name}}">
        @csrf
